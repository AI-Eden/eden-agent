...
On 2026-08-10 the owner accepted the accelerated R3 delivery direction with R3-D changed to a separately activated, non-blocking milestone, then accepted ADR 0019, the focused contract changes below, and `docs/plans/2026-08-10-r3-resume-ready-verified-goal.md` as the complete Freeze packet. After Slice 0 invalidated the one-call and 1 MiB assumptions, the owner accepted the bounded amendment on 2026-08-11, approved the amended Freeze, freshly authorized Build, authorized public-first commits and pushes, accepted the deterministic candidate, and authorized copied packaged TUI evidence plus exactly one matching real-provider/network fixture. Candidate `092f9a107e93112b401a1c9e48dcad04ff064529` passed the deterministic, copied-package, and hosted R2 rows. Its single `deepseek-v4-pro` journey stopped at the explicit `network` retry boundary before completion and was not retried. Offline diagnosis later proved that the old driver had forwarded `NODE_TLS_REJECT_UNAUTHORIZED=0`, so the attempt cannot establish normal TLS; the repaired driver removes that variable and persists sanitized failures, but has not used the network. The milestone review still recommends no R3-A acceptance or closure. `succeeded` remains unavailable and R3-B has not started.
