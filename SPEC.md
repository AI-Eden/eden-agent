...
On 2026-08-10 the owner accepted the accelerated R3 delivery direction with R3-D changed to a separately activated, non-blocking milestone, then accepted ADR 0019, the focused contract changes below, and `docs/plans/2026-08-10-r3-resume-ready-verified-goal.md` as the complete Freeze packet. After Slice 0 invalidated the one-call and 1 MiB assumptions, the owner accepted the bounded amendment on 2026-08-11, approved the amended Freeze, freshly authorized Build, and authorized public-first commits and pushes. The R3-A deterministic implementation candidate is locally green but matching real-provider/network and copied packaged TUI evidence remain `not-run`; R3-A is not accepted or closed, `succeeded` remains unavailable, and R3-B has not started.
