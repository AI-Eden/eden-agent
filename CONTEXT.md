...
- R3-A is owner-accepted and closed. The deterministic and copied packaged TUI rows are green, and the matching real-provider row passed at exact candidate `468c4ba0f726715c2f190b3c2842f798992e8543` under normal TLS. The accepted R3-B Freeze packet is ADR 0020 plus `docs/plans/2026-08-11-r3-b-terminal-product-shell.md`; the Slices 0-4 implementation candidate and copied-package diagnostic are green, while formal exact-source Slice 5 evidence remains pending. The accepted production claim remains R2 until a later release gate, and any commit/push or new provider/network attempt requires fresh owner authority.
