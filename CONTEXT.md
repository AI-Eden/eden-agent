...
On 2026-08-10 the owner accepted the accelerated R3 decisions with one amendment: R3-D remains a bounded read-only ExploreAgent plus web-tools direction but is non-blocking and requires separate activation. The blocking path is R3-A usable coding loop, R3-B TUI product-shell reconstruction, R3-C Plan plus verified Goal, and R3-E resume-ready release. The owner then accepted ADR 0019 and the executable plan and separately authorized the blocking Build plan. Slice 0 stopped before capability activation because the inherited one-call contract could not reach 16 tool calls within a completed 12-step run and the 1 MiB journal estimate had no credible R3 lifecycle headroom. On 2026-08-11 the owner accepted the Freeze amendment: `usable_coding_v1` separates policy maxima, a durable per-run grant, and usage; one step may carry up to four preflighted read-only calls with concurrency at most four; effectful and approval-bearing calls remain singleton; step 12 is final-answer-only; and the profile run-journal ceiling becomes 2 MiB while 64 KiB record and 4096-record limits remain. The owner then approved the amended Freeze, freshly reauthorized Build, authorized public-first commits and pushes, accepted the deterministic candidate, and authorized the copied packaged TUI journey. Candidate `092f9a107e93112b401a1c9e48dcad04ff064529` covers the amended budgets and scheduler, model-visible Git diff, exclusive new-file creation, structured host commands, exact approval/recovery, real temporary-Git/process behavior, replay, ProductView, and TUI command/diff cards. Its copied package passed the complete deterministic coding journey at `60x20`, `80x24`, and `100x30`. The first matching `deepseek-v4-pro` row then failed at the explicit `network` retry boundary without an automatic retry; a bounded offline repair made future failures persist sanitized artifacts and removed inherited `NODE_TLS_REJECT_UNAUTHORIZED=0`, so the old row remains failure history and cannot establish normal TLS. One freshly authorized fixture at exact public candidate `468c4ba0f726715c2f190b3c2842f798992e8543` later passed the complete copied-package journey against `https://api.deepseek.com` and `deepseek-v4-pro` under normal TLS, with 6 exact-usage model attempts, 5 expected tools, 3 approvals, a passing independent repository oracle, terminal restoration, and no credential-canary exposure. Exact-candidate hosted R1 run [`31432887990`](https://github.com/AI-Eden/eden-agent/actions/runs/31432887990) and R2 run [`31432755164`](https://github.com/AI-Eden/eden-agent/actions/runs/31432755164) also passed. The owner accepted and closed R3-A, then selected the R3-B conversation-spine/evidence-lens direction with a minimal-stream/action-overlay narrow fallback and accepted ADR 0020 with `docs/plans/2026-08-11-r3-b-terminal-product-shell.md` as the focused Freeze packet. The owner separately authorized R3-B Build, reviewed the first active-input RED, and authorized autonomous work inside the frozen R3-B boundary. Exact candidate `9dd9e0d9fa8fa3696bfc0e25c129d0e93cb3a8c0` completed the implementation, and the later exact public source `f98e8b3d87b530d46aa7e33664290a02a75ad1a5` retained it with the active-composer escape fix and acceptance-driver hardening. Fresh copied-package evidence bound to that final source passed the three accepted viewports, exact steer then queue delivery, multiline CJK content, eight model attempts, six tools, three approvals, rapid resize, independent repository oracles, zero exit, and parent-terminal restoration with no external network or verifier-success claim. Hosted R1 run [`31498375172`](https://github.com/AI-Eden/eden-agent/actions/runs/31498375172) passed the last product change, and hosted R2 run [`31501285574`](https://github.com/AI-Eden/eden-agent/actions/runs/31501285574) passed the final public evidence head. On 2026-08-11 the owner reviewed this closure chain; R3-B is owner-accepted and closed. R3-C remains not started, and R3-D, another provider call, package publication, and release remain unauthorized.

Current 2026-08-12 status supersedes the preceding historical closeout wording: the owner-operated packaged review exposed trust-focus, readiness-feedback, and interrupted-classification defects, and the owner authorized a bounded matching-surface repair amendment. Exact repair candidate `381b2f404b4f38397831f8193f7dced8efd20ea1` preserves trust focus through the updating transition, makes readiness checking and success observable, and retains classified post-visible provider failures. Its copied-package journeys passed all three accepted viewports, and hosted R1 run [`31590345318`](https://github.com/AI-Eden/eden-agent/actions/runs/31590345318) plus hosted R2 run [`31590345277`](https://github.com/AI-Eden/eden-agent/actions/runs/31590345277) passed. The authorized matching-surface repair amendment is complete, and R3-B is owner-accepted and closed. The owner-observed real-provider interruption remains an explicitly bounded residual failure because no additional provider request was made. R3-C remains not started, and R3-D, another provider call, package publication, and release remain unauthorized.
...
- R3-A is owner-accepted and closed. The deterministic and copied packaged TUI rows are green, and the matching real-provider row passed at exact candidate `468c4ba0f726715c2f190b3c2842f798992e8543` under normal TLS. The accepted R3-B Freeze packet is ADR 0020 plus `docs/plans/2026-08-11-r3-b-terminal-product-shell.md`; refreshed copied-package repair evidence is bound to exact candidate `381b2f404b4f38397831f8193f7dced8efd20ea1`, and the matching-surface repair amendment is complete. R3-B is owner-accepted and closed. R3-C remains not started. The accepted production claim remains R2 until a later release gate; another provider/network attempt, package publication, and release remain separately gated.
