...
- The amended R3 Freeze packet is fixed implementation input. Fresh Build and public-first commit/push authority were granted on 2026-08-11. R3-A deterministic implementation evidence is green, but the separately authorized real-provider and copied packaged TUI rows remain `not-run`; the current accepted production surface therefore remains the completed R2 product described below.
