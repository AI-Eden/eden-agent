...
- Decision authority: this brief records a proposal only; it does not authorize Freeze, Build, commits, pushes, provider or network calls, or release publication

## Decision requested

Decide whether to replace the original R3-plus delivery sequence with five resume-ready vertical milestones: a usable coding loop, a dedicated TUI product-shell reconstruction, Plan plus verified Goal, one read-only ExploreAgent plus bounded web tools, and a packaged v0.1 release journey.
...
If accepted, this brief supersedes only the R3-plus delivery sequencing in the initial research. Accepted ADRs, `SPEC.md`, `PRODUCT.md`, focused documents, approved plans, and `CONTEXT.md` remain authoritative and must be updated through Freeze before implementation begins.
...
### R3-D: Read-only ExploreAgent and Web Tools

Deliver exactly one read-only ExploreAgent before the resume-ready gate. It receives a self-contained assignment, scoped repository reference, independent context and child journal, explicit budgets, cancellable lifecycle, and inherited-but-narrowed capabilities. It returns structured findings with paths, optional lines, evidence, and unknowns.
...
- read, list, search, status, diff, AnchorEdit, new-file write, controlled command, named checks, web search, and web fetch;
- Plan review, revision, approval, and execution;
- Goal verification, bounded repair, durable resume, and Evidence Pack;
- one read-only ExploreAgent;
...
Every milestone must end in a runnable vertical journey. Core invariants remain test-first. Reversible UI composition and routine tool wiring use focused tests and matching-surface QA without requiring a new ADR or a separate architecture interview. Unchanged R0-R2 evidence remains reusable; a full hosted matrix is repeated when an affected platform or package contract changes and at the release-candidate gate.

## Public documents affected after acceptance

Owner acceptance would move this decision into Freeze. Freeze must then:
...
The owner should accept or amend the following as one bounded checkpoint rather than reopen a broad research program:

1. R3-A through R3-E as the current R3 delivery sequence;
2. the R3-A minimum tool and `completed` versus `succeeded` boundary;
3. R3-B as a dedicated OpenTUI product-shell reconstruction;
4. exactly one read-only ExploreAgent plus `web_search` and `web_fetch` before the resume-ready gate;
5. the R3-E public journey as the minimum evidence behind resume-ready claims.

Until that checkpoint is accepted and the resulting Freeze packet is approved, this brief remains research input and authorizes no implementation.
