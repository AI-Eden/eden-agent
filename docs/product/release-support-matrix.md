...
The first owner-authorized `deepseek-v4-pro` R3-A attempt stopped before completion at the explicit `network` retry boundary and was not retried. Later offline diagnosis found that the old driver had forwarded `NODE_TLS_REJECT_UNAUTHORIZED=0`, so that attempt cannot establish normal TLS. The repaired driver removes that variable and persists sanitized failure evidence. One later freshly authorized copied-package fixture at exact candidate `468c4ba0f726715c2f190b3c2842f798992e8543` passed against `https://api.deepseek.com` and `deepseek-v4-pro` under normal TLS, and exact-candidate hosted R1 run 31432887990 and R2 run 31432755164 passed. The owner accepted and closed R3-A. These rows neither reopen the completed R2 claim nor establish R3 release support.

## R3-B milestone evidence

Final source `f98e8b3d87b530d46aa7e33664290a02a75ad1a5` passed the validated copied Linux x64 package journey at `60x20`, `80x24`, and `100x30` after the last R3-B product change. Every journey delivered one multiline CJK steering input and one queued follow-up in exact order, consumed eight model attempts, executed six expected tools with three approvals, passed an independent repository oracle, exited zero, and restored the parent terminal. The wide journey also passed rapid narrow-medium-wide resize. The machine-readable record is `docs/benchmark-results/2026-08-11-r3-b-packaged-tui-local.json`; it used a deterministic local provider fixture, made no external network request, and claims `completed` rather than verifier-owned `succeeded`. Hosted R1 run 31498375172 passed the last product change, and hosted R2 run 31501285574 passed the final evidence source. R3-B is owner-accepted and closed. These rows are R3-B milestone evidence, not release support; R3-C has not started, and another provider/network attempt requires fresh authority.
